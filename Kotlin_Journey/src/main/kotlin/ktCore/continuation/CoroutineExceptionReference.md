...
| 特性 | `coroutineScope` | `supervisorScope` | `runBlocking` | `GlobalScope` |
|------|:---:|:---:|:---:|:---:|
| 类型 | suspend | suspend | 普通函数 | 单例对象 |
| 等待子协程 | ✅ 挂起 | ✅ 挂起 | ✅ 阻塞线程 | ❌ 不管 |
| 子异常→取消兄弟 | 💀 取消 | ✅ 不影响 | 💀 取消 | ✅ 不影响 |
| 子异常→取消自己 | 💀 取消 | ✅ 不影响 | 💀 取消 | N/A 无父级 |
| 子异常→父级 | 💀 re-throw 根因 | ✅ 不影响 | 💀 re-throw 根因 | N/A 无父级 |
| 适用场景 | "一个挂全停" | "各管各的" | 桥接阻塞/挂起 | 不推荐 |
