...
/**
 * 这个语法是 Kotlin 中定义函数式接口（function interfaces）的方式。具体来说，这段代码定义了一个名为 Comparator 的函数式接口，
 * 它接受一个类型参数 T，并且声明了一个抽象函数 compare，该函数接受两个参数 a 和 b，都是类型 T，并返回一个 Int 值。
 * fun interface: 表示这是一个函数式接口，即只包含一个抽象函数的接口。在 Kotlin 中，函数式接口可以被 lambda 表达式或匿名函数所实现。
 */
fun interface FunInterface<T> {
...
class InterfaceTest{
    @Test
    fun test1(){
        // 通过匿名内部类的方式实现一个接口 。 ISwim 也是一个接口，但是不是函数式接口。系统默认可能出现多个抽象方法，故写法很冗余。
        val sw = object : ISwim {
            override fun swim() {
                println(" 我能游")
            }
        ...
        // 使用 lambda 表达式来定义和调用带参数的函数。
        val greet: (String) -> String = { name ->
            val nameLength = name.length
            val greetingMessage = if (nameLength > 5) {
                "Wow, your name is quite long!"
            } else {
                "Your name is short and sweet!"
            }
            "Hello, $name! $greetingMessage (Your name has $nameLength characters.)"
        }
